...
import config from '../../config/webSettings';
    ...
    return (
      <html lang="en-US">
        <head>
          ...
          <meta name="application-name" content="Evangelize" />
          <meta name="apple-mobile-web-app-status-bar-style" content="black" />
          <meta name="apple-mobile-web-app-title" content="Evangelize" />
        ...
        <body>
          ...
          {store && (
            <script
              dangerouslySetInnerHTML={{ __html: `window.store=${serialize(store)};` }}
              charSet="UTF-8"
            />
          )}
          {vars && vars.map(variable => variable && (
            <script
              dangerouslySetInnerHTML={{ __html: `window.${variable.name}=${serialize(variable.data)};` }}
              charSet="UTF-8"
            />
          ))}
