...
export default class Jobs {
  ...
  getJobMembers(id) {
    const jobMembers = this.db.store.filter(
  ...
  getWorshipServiceJobs(serviceId) {
    const serviceJobs = this.db.store.filter(
      'worshipServiceJobs',
      [
    ...
    const jobs = this.db.store.filter(
      'jobs',
      [
        filter((rec) => rec.deletedAt === null &&
  ...
  getServiceJobMembersCount(serviceId, jobId, date) {
    const assignments = this.db.store.filter(
  ...
  getServiceJobMembers(serviceId, jobId, date) {
    let members = [];
    const job = this.db.store.filter(
      'jobs',
      [
        filter((rec) => rec.deletedAt === null &&
          rec.id === jobId),
  ...
  @action updateMemberJobAssignment(job, service, date, oldPerson, newPerson) {
    ...
    let newRecord;
    const ts = moment();
    const record = (oldPerson.id) ? this.db.store.filter(
      'memberJobAssignments', [
        filter((rec) => rec.deletedAt === null && 
